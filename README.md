...
### Database Import with XAMPP

If you are running the project locally with XAMPP, start both **Apache** and **MySQL** first, then import the database dump before launching the app.

1. Open the XAMPP control panel and start **Apache** and **MySQL**.
2. Open `phpMyAdmin` at `http://localhost/phpmyadmin`.
3. Create a database named `cod_crm`.
4. Import [setup_database.sql](setup_database.sql) into that database to create the schema and base demo data.
5. If you want the larger synthetic dataset we used for training and testing, import [seed_data.sql](seed_data.sql) after the schema is created. It contains the auto-generated order history used for ML experiments.

`setup_database.sql` is the clean starter database. `seed_data.sql` is the full synthetic seed set.
...
If you imported [setup_database.sql](setup_database.sql) through XAMPP, the database is already ready. The backend is configured to run on port `8080`, which matches the frontend's default API target.
...
If you need to point the frontend manually, use `NEXT_PUBLIC_API_URL=http://localhost:8080/api/v1`.
...
curl -X POST http://localhost:8080/api/v1/ai/retrain \
